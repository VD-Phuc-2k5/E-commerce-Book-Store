...
async function getBooks(n) {
  ...
  // Tạo một Set chứa id của các sách bán chạy để kiểm tra trùng lặp
  const topSellingIds = new Set(topSellingBooks.map((book) => book.id));

  // Lọc ra các sách không nằm trong danh sách bán chạy
  const otherBooks = books.filter((book) => !topSellingIds.has(book.id));
